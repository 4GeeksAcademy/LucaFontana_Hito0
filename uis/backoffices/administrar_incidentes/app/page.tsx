...
const categories = ["equipment_failure", "supply_issue", "customer_complaint", "staff_issue", "facility_issue", "pos_system", "delivery_issue", "other"] as const;
const categoryLabels: Record<string, string> = { equipment_failure: "Falla de equipo", supply_issue: "Suministros", customer_complaint: "Queja de cliente", staff_issue: "Personal", facility_issue: "Instalaciones", pos_system: "Sistema POS", delivery_issue: "Domicilios", other: "Otro" };
const statuses = ["open", "in_progress", "resolved", "discarded"] as const;
const statusLabels: Record<string, string> = { open: "Abierto", in_progress: "En progreso", resolved: "Resuelto", discarded: "Descartado" };
type Status = (typeof statuses)[number];
type Incident = { id: string; title: string; description: string; category: string; status: Status; origin: string; branch: string; created_at: string };
type Filters = { status: string; origin: string; branch: string; category: string };
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) { return <label className="field"><span>{label} <b>*</b></span>{children}{error && <small className="field-error">{error}</small>}</label>; }
function Metric({ label, value, tone }: { label: string; value: number | string; tone: string }) { return <div className={`metric ${tone}`}><span>{label}</span><strong>{value}</strong></div>; }
