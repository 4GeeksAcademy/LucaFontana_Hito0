import { MenuItem, SaleTransaction, Localizacion, MenuCategory, LocationStatus } from "../types/models";
...
export function filterActiveLocations(locations: Localizacion[]): Localizacion[]{
    return locations.filter(location => location.status === "Active");
}
//Retorna todas las localizaciones según un estado específico (Función customizada)
export function filterCustomLocations(locations: Localizacion[], estado: LocationStatus): Localizacion[]{
...
export function sortLocationsByCapacity(locations: Localizacion[], order: "asc" | "desc"): Localizacion[]{
    if (order === "asc") {
